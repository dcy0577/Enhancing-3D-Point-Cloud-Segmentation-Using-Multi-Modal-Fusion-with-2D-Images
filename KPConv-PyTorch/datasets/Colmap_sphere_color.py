...
from datasets.Colmap_get_rgbd_overlap_subcloud import compute_rgbd_knn_colmap
...
# --------------------------- #
class ColmapDataset(PointCloudDataset):
    """Class to handle custom colmap dataset."""
    ...
    def __init__(self,
        ...
        """
        PointCloudDataset.__init__(self, 'Colmap')
        ############
        # Parameters
        ############
        ...
        # Dataset folder path
        self.path = config.colmap_dataset_path
        # camera parameters
        self.parameter_dir = config.parameter_dir
        # includes depth images
        self.depth_image_dir = config.depth_image_dir
        # includes color images
        self.color_image_dir = config.color_image_dir
        # point cloud path
        self.point_cloud_path = config.point_cloud_path
        ...
        if self.set == 'training':
            ...
            raise ValueError('Unknown set for the data: ', self.set)
        ...
        #     depth_size = (1800, 1000)  # intrinsic matrix is based on unstabiled depth maps.
        ...
        self.files = []
        self.cloud_names = ['cms_library_label']
        # self.cloud_names = ['cms_Alex_label', 'cms_Prof_label','cms_library_label']
        # self.cloud_names = ['cms_Alex_label', 'cms_Prof_label']
        # self.cloud_names = ['cms_Alex_label']
        # self.cloud_names = ['cms_Prof_dummy_label']
        for i,f in enumerate(self.cloud_names):
            if self.set == 'validation':
                self.files += [join(self.path, f, self.point_cloud_path, f + '.ply')]
            else:
                raise ValueError('this dataset currently cant support training')
    ...
    def _get_colmap_paths(self, root_path, cloud_name):
        return {'color': osp.join(root_path, cloud_name, self.color_image_dir, '{}'),
                 # 'depth': osp.join(root_path, cloud_name,self.depth_image_dir, '{}.photometric.bin'),
                 'depth': osp.join(root_path, cloud_name,self.depth_image_dir, '{}.geometric.bin'),
                 'pose': osp.join(root_path, cloud_name,self.parameter_dir, 'images.bin'),
                 'camera_intrinsics': osp.join(root_path, cloud_name,self.parameter_dir, 'cameras.bin'),
                'translation_matrix_for_images': osp.join(root_path, cloud_name,self.parameter_dir, 'matrix_for_images.txt')
                 }
    ...
    def get_rgbd_data(self, sub_rgbd_dict, input_sphere_points, input_sphere_mask, input_sphere_label, whole_scene_points, whole_scene_label):
        ...
        cloud_name = sub_rgbd_dict['cloud_name']
        paths = self._get_colmap_paths(self.path, cloud_name)
        ...
        for i, frame_id in enumerate(selected_frames):
            ...
            # load color image
            image = Image.open(paths['color'].format(image_info_dic[frame_id].name))
            ...
            if self.resize:
                ...
                cam_matrix[0] /= depth_map_size[0] / self.resize[0]
                cam_matrix[1] /= depth_map_size[1] / self.resize[1]
            ...
            if self.resize:
                if not image.size == self.resize:
                    ...
                    depth_im = depth_im.resize(self.resize, Image.NEAREST)
            ...
            # rescale depth (dont need for colmap)
            ...
            image_xyz = np.matmul(image_xyz - t, rotation_matrix)

            # aligns unproject points with scan point cloud
            matrix = np.loadtxt(paths['translation_matrix_for_images'], dtype=np.float32)
            ones = np.ones(shape=image_xyz.shape[0])
            image_xyz = np.append(image_xyz.T, [ones], axis=0)
            image_xyz = np.matmul(image_xyz.T, matrix.T)
            image_xyz = np.delete(image_xyz, 3, axis=1)


            if not np.any(image_mask):
                print('Invalid depth map for frame {} of {}.'.format(frame_id, cloud_name))
        ...
        if _DEBUG == True:
            ...
            # o3d.io.write_point_cloud(path + cloud_name + '_input_sphere_points' + '.ply', pts_vis)
            # o3d.io.write_point_cloud(path + cloud_name + '_base_points' + '.ply', base_pts_vis)
            # o3d.io.write_point_cloud(path + cloud_name + '_image_xyz_points' + '.ply', pcd_xyz)
            o3d.io.write_point_cloud(path + cloud_name + '_whole_scene_points' + '.ply', pcd_scene)
        # -----------------------------------------------------------------------------------------------
    ...
    def potential_item(self, batch_i, debug_workers=False):
        ...
        while True:
            ...
            with self.worker_lock:
                ...
                assert check_scan_id == self.sub_rgbd_dict[cloud_ind]['cloud_name'], 'Mismatch scan_id: {} vs {}.'.format(check_scan_id, self.sub_rgbd_dict[cloud_ind]['cloud_name'])
            ...
            # if can not find image points, kill the current loop and start next loop
            try:
    ...
    def load_subsampled_clouds(self):
        ...
        for index, file_path in enumerate(self.files):
            ...
            if exists(KDTree_file):
                ...
                base_point_ind, pointwise_rgbd_overlap, frame_ids = compute_rgbd_knn_colmap(cloud_name, cameras_dic, image_info_dic, paths, sub_points)  # cache_data return whole scence points

                sub_rgbd_dict = {'cloud_name' : cloud_name}
...
class ColmapSampler(Sampler):
    """Sampler for S3DIS"""

    def __init__(self, dataset: ColmapDataset):
...
def ColmapCollate(batch_data):
    return ColmapCustomBatch(batch_data)
