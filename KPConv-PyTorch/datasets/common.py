...
class PointCloudDataset(Dataset):
    ...
    def augmentation_transform_new(self, points, image_xyz, normals=None):
        """Implementation of an augmentation transform for original point clouds and unprojected point cloud."""
        ...
        # augmented_points = np.dot(points, R) * scale + noise
        augmented_points = np.sum(np.expand_dims(points, 2) * R, axis=1) * scale + noise
        original_shape_of_image_xyz = image_xyz.shape
        augmented_image_xyz = np.sum(np.expand_dims(image_xyz.reshape([-1, 3]), 2) * R, axis=1) * scale
        augmented_image_xyz = augmented_image_xyz.reshape(original_shape_of_image_xyz)

        if normals is None:
            return augmented_points, scale, R, augmented_image_xyz
        else:
            # Anisotropic scale of the normals thanks to cross product formula
            normal_scale = scale[[1, 2, 0]] * scale[[2, 0, 1]]
            augmented_normals = np.dot(normals, R) * normal_scale
            ...
            return augmented_points, augmented_normals, scale, R, augmented_image_xyz
 ...
 #-----------------NEW 5.7.2021---------------------------------------
    def segmentation_inputs_sphere(self,
        ...
        # list of network inputs
        li = input_points + input_neighbors + input_pools + input_upsamples + input_stack_lengths
        li += [stacked_feature_points,stacked_image_xyz,stacked_images,labels]
